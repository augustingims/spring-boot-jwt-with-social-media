...
public class UserServiceIT {
    ...
    public void assertThatAnonymousUserIsNotGet() {
        user.setLogin(ConstantsUtils.ANONYMOUS_USER);
        if (!userRepository.findOneByLogin(ConstantsUtils.ANONYMOUS_USER).isPresent()) {
            userRepository.saveAndFlush(user);
        }
        final PageRequest pageable = PageRequest.of(0, (int) userRepository.count());
        final Page<UserDTO> allManagedUsers = userService.getAllManagedUsers(null, pageable);
        assertThat(allManagedUsers.getContent().stream()
            .noneMatch(user -> ConstantsUtils.ANONYMOUS_USER.equals(user.getLogin())))
            .isTrue();
    }
